...
const rateHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (
    ...
    metrics.ramp_up === undefined ||
    metrics.responsiveness === undefined ||
    metrics.license === undefined ||
    metrics.pull_req_score === undefined ||
    metrics.dependency_score === undefined
  ) {
  ...
  if (user) {
    ...
    await prisma.action.create({
      data: {
        username: user.username ? user.username : (user.name as string),
        action: "RATE",
